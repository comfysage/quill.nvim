...
---@return quill.config.signature
local cfg = function()
	return _G.quill.config.signature
end
...
M.setup = function(client, buf)
	...
	local groupid = vim.api.nvim_create_augroup(string.format("QuillSignature[buf=%d]", buf), {
		clear = true,
	})
	vim.api.nvim_create_autocmd("InsertLeave", {
		group = groupid,
		buffer = buf,
		callback = require("quill.signature").close_signature,
	})
	vim.api.nvim_create_autocmd({ "InsertEnter", "CursorMovedI" }, {
		group = groupid,
		buffer = buf,
		callback = require("quill.signature").auto_signature,
	})
end
